...
# create rc config
chmod +x tasks/deploy/group-controller.yml.sh && tasks/deploy/group-controller.yml.sh
...
if [ $($KUBERNETES_CMD get rc | grep -c group) -ne 1 ]; then
    echo "Create group rc with replicas:" $REPLICAS_NUMBER
    $KUBERNETES_CMD create -f tasks/deploy/group-controller.yml
else
    echo "Rolling update authorizer rc"
    $KUBERNETES_CMD rolling-update group --update-period=10s --image=${EXTERNAL_REGISTRY_ENDPOINT}/group:${CIRCLE_BRANCH}.${CIRCLE_SHA1}
fi
